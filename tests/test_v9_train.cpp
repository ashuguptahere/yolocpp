// v9 train smoke — finetune yolo9c on coco8 for 2 epochs, assert that
// loss decreases and the trainer writes a checkpoint. Skipped if the
// converted weights or coco8 are not available.
//
// v9 plugs into TrainerT<Yolo9> via the default LossTraits<M> →
// V8DetectionLoss specialisation — the GELAN backbone feeds a v8-style
// anchor-free DFL Detect head (legacy=true, reg_max=16), so no v9-specific
// loss class is needed.
...
int main() {
  ...
  if (!fs::exists("data/coco8/images/train")) {
    std::cout << "[v9-train] SKIP: data/coco8 missing\n";
    return 0;
  }

  std::string home = std::getenv("HOME") ? std::getenv("HOME") : "/tmp";
  fs::path w = fs::path(home) / ".cache/yolocpp/weights/yolo9c.pt";
  if (!fs::exists(w)) {
    std::cout << "[v9-train] SKIP: " << w << " not present\n";
    return 0;
  }
  ...
  cfg.lr0        = 0.001;
  cfg.device     = "cpu";
  cfg.save_dir   = "/tmp/yolocpp_v9_train_smoke";
  cfg.log_every  = 100;  // suppress per-step logging
  cfg.warmup_epochs = 0;

  fs::remove_all(cfg.save_dir);
  engine::TrainerV9 trainer(model, train_ds, cfg);
  trainer.run();

  // Trainer writes last.pt + best.pt under save_dir/<run-name>/.
