...
Yolo8ClassifyImpl::Yolo8ClassifyImpl(Yolo8Scale s, int nc_)
    : scale(s), nc(nc_) {
  // The upstream yolo8-cls.yaml uses max_channels=1024 for ALL scales
...
int Yolo8ClassifyImpl::load_from_state_dict(
  ...
  if (skipped_shape > 0)
    std::cerr << "[classify load] skipped " << skipped_shape
              << " tensors with shape mismatch (final fc re-purposed for custom nc)\n";
  return copied;
}
