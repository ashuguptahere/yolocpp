...
| #65F2 | ~~Real-arch forward end-to-end.~~ Partial 0.43.0 — `RFDetrImpl::forward_eval` runs full backbone+projector+two-stage+iterative-decoder pipeline; YOLO `[B,4+nc,Q]` shape contract holds. **Bit-exact numerical parity NOT yet achieved** (0 detections at conf=0.05); details in `#65L`. Remaining: sinusoidal embed order, windowed-attn on backbone, decoder self-attn v=tgt vs v=tgt+pos. | partial (0.43.0) | 0.5 session remaining | #65L |
