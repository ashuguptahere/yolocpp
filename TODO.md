...
| #51B | ❌ won't fix — evaluated 2026-05-02 after #51A landed. **Decision:** keep both styles first-class. Reasoning: (a) kv-style (`task=detect mode=train ...`) is the drop-in interface that lets users port pipelines verbatim from upstream tooling — removing it adds friction for the largest cohort of new users; (b) subcommand-style (`yolocpp train -m ... -d ...`) is for users who prefer conventional Unix flags. Both reach the same `cmd_*` bodies (no duplicated logic since #46F3), so maintaining both isn't a code-debt issue. (c) The original "removal or deprecation" framing assumed the two styles were redundant — they aren't; they serve different audiences. Re-open only if a concrete reason to remove emerges (e.g., conflicting flag interpretation between the two). | won't fix | n/a | — |
