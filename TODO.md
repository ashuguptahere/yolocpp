...
| #65A2 | Replace `rfdetr_backbone.{hpp,cpp}` with HF-DINOv2 windowed-attn (12 blocks, embed=384, separate Q/K/V, layer_scale1/2). Match upstream key names exactly (`backbone.0.encoder.encoder.encoder.layer.<i>.attention.attention.{query,key,value}`). | high | 1 session | — |
| #65B2 | Add `rfdetr_projector.{hpp,cpp}` (CSP-style, single output level P4) matching `backbone.0.projector.stages.0.0.{cv1,cv2,m.<i>}`. | high | 0.5 session | #65A2 |
| #65C2 | Rewrite `rfdetr_decoder.{hpp,cpp}`: fused-QKV self-attn (`in_proj_weight [3·hidden, hidden]`), single-level deformable cross-attn (`num_levels=1, dec_n_points=2, ca_nheads=16`), shared cls/bbox heads with iterative bbox refinement. Match upstream `transformer.decoder.layers.<j>.{self_attn, cross_attn, linear1/2, norm1/2/3}`. | high | 1 session | #65A2,B2 |
| #65D2 | Add `transformer.enc_output*` two-stage encoder-output module (13 layer-aware cls + bbox heads on backbone taps for query initialisation). | high | 0.5 session | #65A2,C2 |
| #65E2 | `rfdetr_weights.{hpp,cpp}` — direct loader from upstream `.pt` / `.pth`. Since key names are matched 1-to-1 by the rewrite, this is just dtype + shape gates + `register_parameter` lookups (no key remapping). Wire `RFDetrImpl::load_from_state_dict`. | high | 0.5 session | #65A2..D2 |
| #65F2 | End-to-end test: load `rf-detr-base.pth`, forward `data/bus.jpg`, verify ≥ 1 plausible detection (top-1 score > 0.5, box inside frame). Repeat for all 5 detect variants. | high | 0.5 session | #65E2 |
