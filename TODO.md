...
| #52A2 | Thread per-image dets back from `predict_to_file` adapters so `YOLO::predict` returns populated `vector<Detection>`. Currently the API returns `{}` and users read `runs/predict/<stem>.jpg`. Touching every adapter; defer until a use case asks for it. | low | within #52 | — |
