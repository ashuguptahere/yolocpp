...
**Round 8 (0.107.7) — older-version forwards + emitters, 0 candidates.** A 6-finder hunt over the last code surface not yet swept this session — the older-version model forward decode + ONNX/TRT emitters (v3/v5 anchor-free DFL, v4 Darknet scale_xy, v6 Meituan EfficientRep/V6ActScope, v7 ELAN/IDetect/ReOrg, v9 GELAN/CBFuse, v10 NMS-free + v1/v2 Darknet-era) — surfaced **0 candidate bugs** (nothing even reached verification). Consistent with these passing per-layer parity + the 164/164 full-matrix sweep + cross-backend PT↔TRT parity. **This completes a full-codebase adversarial sweep across rounds 6–8** (new task code, mature core, periphery, older versions): the only real bugs were the 2 nc-handling defects in the untested CLI/API periphery, now fixed (0.107.3/0.107.4/0.107.6).

**Round 7 (0.107.3–0.107.4) — periphery sweep, 2 confirmed + fixed.** A 33-agent hunt (6 finders × 3-lens refute-by-default) over the *less-parity-tested* surface — CLI parse/dispatch, web console, dataset/yaml resolution, Darknet `.weights` converters, registry/adapter, public API — surfaced **9 candidates → 2 confirmed real bugs**, both in the CLI/API nc-handling (vindicating the choice to hunt off the parity harness): (1) **export silently used nc=80** for non-COCO models (`export_after_train` CLI+API + bare `--mode export`) → untrained 80-class head; fixed by inferring nc from the checkpoint head in `cmd_export` (0.107.3). (2) **explicit `--nc 80` ignored** for classify/obb predict (the `(nc<0||nc==80)` overload) → silently overridden to 1000/15; fixed with the `nc<0` sentinel (0.107.4). 7 candidates rejected on verification — incl. a web-server "path traversal" (localhost-bound; the model param loads-as-weights, never serves file contents; `/runs` mount uses httplib's traversal guard) and a `scale_from_filename` "e6 before e6e" claim that's false (the code checks `-e6e` first). Hand-verified the lone split-vote + the concrete rejections myself.
