...
| #51B | 🟡 reopened+partial — maintainer requested a flat flag-style canonical shape (`yolocpp --mode <action> -m ... -s ...`). Landed under #51J. Subcommand-style is now formally deprecated (one-line warning + still works) and will be removed under #51B2 once a release window has passed. kv-style stays untouched. |  — | within #51 | — |
| #51B2 | Remove subcommand-style entirely once one release has passed since #51J. Strip the `app.add_subcommand("train", ...)` block + the deprecation warning. Update CLAUDE.md / README to drop the deprecated examples. | low | within #51 | needs maintainer "go" after #51J ships |
| #51J | ✅ closed — flat flag-style canonical CLI: `yolocpp --mode <train\|predict\|val\|export\|benchmark\|info\|download> ...` with every option as a top-level flag. Routing precedence: kv-style → flag-style (when `--mode` present) → subcommand-style (deprecated fallback). Per-mode required-flag validation centralised in `cmd_dispatch_flag_style` so error messages are uniform (`[error] --mode=predict needs --source`). All 7 modes verified end-to-end with real models; legacy kv-style and subcommand-style continue to work. | — | landed | — |
