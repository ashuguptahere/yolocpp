...
//   model=yolov8n.pt   → looks in cwd, ./data, ~/.cache/yolocpp/weights;
...
// Infer YOLOv8 scale ("n"/"s"/"m"/"l"/"x") from a weights filename like
// "yolov8s-seg.pt". Returns empty string if not recognised.
std::string scale_from_filename(const std::string& path);

// Infer YOLO version family ("v3", "v5", "v8", "v9", "v10", "v11", "v12",
// or "rtdetr") from a weights filename. Returns "v8" as a permissive
// default — most Ultralytics-style anchor-free weights load through the
// v8 path since their Detect head shares the same DFL layout.
